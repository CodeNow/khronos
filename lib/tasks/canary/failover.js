...
const DOCK_UNHEALTHY_QUEUE_NAME = 'on-dock-unhealthy'
  ...
  test () {
    ...
    return this.setup()
      .then(() => {
        ...
        .tap((instances) => {
          console.log('Found ' + instances.length + ' non-isolated instances')
        })
        .then((instances) => {
        ...
        .then(() => {
          const largestDock = Object.keys(dockerHosts).sort((a, b) => {
        ...
        .then((largestDock) => {
          return Promise.using(
            rabbitmqHelper([DOCK_UNHEALTHY_QUEUE_NAME]), (rabbitmq) => {
