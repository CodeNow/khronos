...
module.exports = function logCanary (job) {
  ...
  return api.connect(process.env.CANARY_API_TOKEN).then(function (client) {
    var connectSid = client.client.opts.jar._jar.store.idx['runnable-beta.com']['/']['connect.sid'].value;
    return Promise.resolve()
      ...
      .then(function testLogs (instance) {
        ...
        return Promise.race([Promise.all([ testBuildLogs(), testCmdLogs(), testTerminal() ]), failureHandler])
      })
