...
class GithubBranchCanary extends CanaryBase {
  ...
  test () {
    return this.setup()
      ...
      .then((rawInstanceData) => {
        this.log.debug(rawInstanceData, 'Fetched instance')
        const instance = this.client.newInstance(rawInstanceData)
        const status = instance.status()
        this.log.debug({status}, 'Instance status')
        if (status !== 'running') {
      ...
      .then((instance) => {
        ...
        const getCommitsTimer = datadog.timer('github.getCommits')
        return Promise.fromCallback((cb) => {
          ...
          .then((commits) => {
            ...
            const createReferenceTimer = datadog.timer('github.createReference')
            return Promise.fromCallback((cb) => {
      ...
      .then(() => {
        const fetchInstancesTimer = datadog.timer('api.fetchInstances')
  ...
  teardown () {
    ...
    this.log.debug({ url: keypather.get(this, 'ref.url') }, 'Deleting branch from github')
    const delAsyncTimer = datadog.timer('github.delAsync')
