...
class GithubBranchCanary extends CanaryBase {
  ...
  test () {
    return this.setup()
      ...
      .then((instance) => {
        ...
        this.log.debug('Getting all commits', { branchName: this.refName })
        return Promise.fromCallback((cb) => {
          ...
          .then((commits) => {
            ...
            this.log.debug('Creating new branch from commit', { branchName: this.refName, commit: this.lastCommitSha })
            return Promise.fromCallback((cb) => {
  ...
  teardown () {
    ...
    return request.delAsync({
      ...
      .catch((err) => {
        this.log.debug('Test teardown failed. Ignoring error.', {err: err})
      })
  }
}
