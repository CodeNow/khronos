...
const runNetworkContainer = (dockerUrl, containersData) => {
  ...
  return docker.runContainer(PING_IMAGE, cmd)
}
...
class NetworkOrgCanary extends CanaryBase {
  constructor (job) {
    ...
    this.queue = 'canary.network-ping.run'
  ...
  test () {
      ...
      return swarm.checkHostExists(this.job.targetDockerUrl)
        .catch(Swarm.InvalidHostError, (err) => {
          rabbitmq.publishEvent('dock.lost', {
            host: this.job.targetDockerUrl,
            githubOrgId: this.job.targetOrg
          })
    ...
    .then((containersData) => {
      ...
      return runNetworkContainer(this.job.targetDockerUrl, containersData)
        .catch((err) => {
          this.log.error({ err }, 'failed to run container')
          throw new CanaryFailedError('Error trying to ping', { err: err })
        })
        .spread((data, pingContainer, pingLog) => {
          this.log.trace({ data, pingLog }, 'network container result')
        ...
        .spread((data, pingContainer, pingLog) => {
          ...
          if (data.StatusCode !== 0) {
            this.log.error({ data, pingLog, containerId }, 'ping failed')
          ...
          if (pingLog.includes('ERR')) {
            this.log.warn({ pingLog, containerId }, 'ping had errors')
          ...
          this.log.trace({ pingLog, containerId }, 'ping complete')
        })
    })
  }
}
