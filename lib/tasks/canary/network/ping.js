  ...
  test () {
    ...
    .then(() => {
      return Promise.using(mongodbHelper(), (mongoClient) => {
        const contextVersionDb = mongoClient.db.collection('contextVersions')
        return Promise.fromCallback((cb) => {
          log.trace('querying mongo')
          contextVersionDb.find({
            _id: {
              $in: this.job.targetCvs
            }
          }, cb)
        })
        .then((cvs) => {
          this.log.debug('Filtering container that were run on removed dock')
          const ipsToCvs = _.zip(this.job.targetIps, cvs)
          const activeIpsToCvs = ipsToCvs.filter((pair) => {
            return pair && pair[1] && !pair[1].dockRemoved
          })
          const ips = activeIpsToCvs.map(_.first)
          return ips
        })
        .catch((err) => {
          throw new TaskFatalError(
      ...
      return Promise
        ...
        .spread((data, container, pingLog) => {
          ...
          return Promise
            ...
            .return([data, container, pingLog])
        })
        .spread((data, container, pingLog) => {
          var containerId = keypather.get(container, 'id')
