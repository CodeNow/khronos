...
class NetworkOrgCanary extends CanaryBase {
  ...
  test () {
    let contaienrLog = ''
    this.log.debug('Running ping canary')
    return Promise.fromCallback((cb) => {
      this.log.debug('Pulling ping container')
    ...
    .then(() => {
      ...
      return Promise.fromCallback((cb) => {
        const cmd = ['bash', '-c', process.env.RUNNABLE_WAIT_FOR_WEAVE + 'node index.js ' + this.targetIps]
