...
module.exports = function (job) {
  ...
  return Promise.try(() => {
    if (!exists(job.orgId)) {
      throw new TaskFatalError('khronos:metrics:report-org-container-status', 'orgId is required')
    }
    if (!exists(job.orgName)) {
      throw new TaskFatalError('khronos:metrics:report-org-container-status', 'orgName is required')
    }
    if (!exists(job.instances)) {
      throw new TaskFatalError('khronos:metrics:report-org-container-status', 'instances is required')
    }
    if (!exists(job.totalInstances)) {
      throw new TaskFatalError('khronos:metrics:report-org-container-status', 'totalInstances is required')
    }
  })
    .then(() => {
      var masterStati = {
    ...
    .then(() => {
      return intercomClient.companies.listBy({ company_id: job.orgName })
