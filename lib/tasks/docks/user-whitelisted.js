...
const schema = joi.object({
  ...
  createdAt: joi.number().required() // CreatedAt is in seconds
}).required().label('job')
  ...
  return Promise
    ...
    .then(() => {
     // Check if this job is ready
      ...
      var timeLeft = now - (job.createdAt + process.env.CHECK_ASG_CREATED_DELAY_IN_SEC)
      if (timeLeft > 0) {
        // It hasn't been long enough, so let the exponential backoff handle repeating this job
        throw new TaskError(
          'khronos:user-whitelisted',
    ...
    .then((docks) => {
      ...
      if (possibleDocks.length === 0) {
        ...
        throw new TaskFatalError('No Docks exist!')
      }
    })
    .catch((err) => {
      log.error({ err: err }, 'Error in UserWhitelisted')
      throw err
    })
}
