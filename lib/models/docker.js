...
module.exports = class Docker extends DockerClient {
  ...
  runContainer (image, cmd) {
    ...
    return Promise.fromCallback((cb) => {
      this.client
        .run(image, cmd, false, (err, data, pingContainer) => {
  ...
  getContainers (queryOpts, imageBlacklist, containerIdWhitelist) {
    ...
    return this.listContainersAsync(queryOpts)
  ...
  getImages (minAgeSeconds, cb) {
    ...
    return this.listImagesAsync({ all: true })
      .then(function (images) {
        ...
        images = images.filter(function (image) {
          var tag = keypath.get(image, 'RepoTags[0]')
          if (tag && ~tag.indexOf('\u003cnone\u003e')) {
        ...
        images = images
          .map(function (image) {
            if (!image.RepoTags || !image.RepoTags.length) { return image }
  ...
  removeContainer (containerId) {
    ...
    return this.killContainerAsync(containerId)
      .catch((err) => {
        if (/container.+is not running/i.test(err.message)) {
          log.info(
            { err: err },
            'removeContainer: killContainer error: not running'
          )
          return
        }
        log.warn({ err: err }, 'removeContainer kill error')
        throw err
      })
      .then(() => {
        return this.removeStoppedContainer(containerId)
          .catch((err) => {
            log.warn({ err: err }, 'removeStoppedContainer error')
            throw err
          })
      })
  }
