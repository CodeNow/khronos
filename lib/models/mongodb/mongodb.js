  ...
  Mongodb.prototype[functionName] = function (query, cb) {
    ...
    collection
      ...
      .toArray(function (err) {
        if (err) {
          debug.log('failed to fetch: '+collectionName, err);
        }
  ...
  Mongodb.prototype[functionName] = function (query, cb) {
    ...
    collection
      .count(query, function (err) {
        if (err) {
          debug.log('count failed: '+collectionName, err);
        }
        datadog.endTiming(timingKey);
        cb.apply(this, arguments);
      });
  ...
  Mongodb.prototype[functionName] = function (query, cb) {
    ...
    collection
      .remove(query, function (err) {
        if (err) {
          debug.log('remove failed: '+collectionName, err);
        }
        datadog.endTiming(timingKey);
        cb.apply(this, arguments);
      });
  ...
  Mongodb.prototype[functionName] = function (models, cb) {
    ...
    collection
      .insert(models, function (err) {
        if (err) {
          debug.log('error inserting: '+collectionName, err);
        }
        datadog.endTiming(timingKey);
        cb.apply(this, arguments);
      });
