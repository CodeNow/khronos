...
/**
 * Fetch the list of dock ids from Mavis.
 * Allows the overriding of this behavior via the environment variable
 * `KHRONOS_DOCKS`, which should be a comma-separated list of hosts
...
Mavis.prototype.getDocks = function () {
  return this.getRawDocks()
    .then(function (docks) {
      ...
      log.trace({ docks: docks }, 'Mavis.getDocks available docks')
      return docks
    })
}
    ...
    .then(function getDocks () {
      ...
      return request.getAsync(this.host)
        .spread(function (res, body) {
          ...
          log.trace({ docks: docks }, 'Mavis.getRawDocks available docks')
          return docks
        })
    })
    .catch(function (err) {
      log.error({ err: err }, 'Error in Mavis.getRawDocks')
      throw err
    })
}
